      ...
      {!loading && donations.length > 0 && (
        <div className="w-full max-w-md mt-4 space-y-4">
          {donations.map((donation, idx) => (
            <div key={donation.id || `${donation.recipient_name || donation.recipient || 'N/A'}-${donation.amount}-${donation.date || donation.created_at || idx}`}
              className="bg-white p-4 rounded shadow flex flex-col">
              <div className="font-semibold">Recipient: {donation.recipient_name || donation.recipient || 'N/A'}</div>
              <div>Amount: ${donation.amount}</div>
              <div>Date: {donation.date || donation.created_at || 'N/A'}</div>
              <div>Status: {donation.status || 'Completed'}</div>
            </div>
          ))}
        </div>
      )}
